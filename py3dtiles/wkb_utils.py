...
class TriangleSoup:
    ...
    def from_wkb_multipolygon(wkb, associatedData = []):
        ...
        trianglesArray = [[] for i in multipolygons]
            ...
            if(len(polygon) != 1):
                ...
                if(len(polygon[0]) > 3):
                    triangles = triangulate(polygon[0],
                        [ap[0] for ap in additionalPolygons])
                    for array, triangle in zip(trianglesArray, triangles):
                        array += triangle
                else:
                    for array, triangle in zip(trianglesArray,
...
def parse(wkb):
    ...
    # print(byteorder)
    geomtype = struct.unpack('I', wkb[1:5])[0] # 1006 (Multipolygon Z)
    hasZ = geomtype == 1006                    # 6 Multipolygon
    ...
    for i in range(0, geomNb):
        ...
        for j in range(0, lineNb):
            ...
            for k in range(0, pointNb-1):
                point = np.array(struct.unpack(pntUnpack, wkb[offset:offset+pntOffset]),
                                 dtype=np.float32)
                offset += pntOffset
                line.append(point)
...
def triangulate(polygon, additionalPolygons = []):
    ...
    arrays = [[] for i in range(0, len(additionalPolygons) + 1)]
