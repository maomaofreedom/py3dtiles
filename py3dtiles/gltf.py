...
class GlTF(object):
    ...
    def to_array(self):  # glb
        ...
        padding = np.array([0 for i in range(0, (4 - len(self.body) % 4) % 4)], dtype=np.uint8)
        ...
        binChunkHeader = np.array([len(self.body) + len(padding), # BIN chunck length
    ...
    @staticmethod
    def from_binary_arrays(arrays, transform, binary=True, batched=True, uri=None, textureUri = None):
        """
        Parameters
        ----------
...
def compute_header(binVertices, nVertices, bb, transform,
    ...
    if batched:
        bufferViews.append({
            'buffer': 0,
            'byteLength': int(round(sum(sizeVce) / 3)),
            'byteOffset': int(round(8 / 9 * sum(sizeVce))) if textured else 2 * sum(sizeVce),
            'target': 34962
        })
